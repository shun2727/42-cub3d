...
/*   Updated: 2026/09/07 21:48:05 by syee             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_str(char *staticbuffer)
{
	...
	if (newline_ptr == NULL)
	{
		returnstr = gnl_strdup(staticbuffer);
		if (!returnstr)
			return (NULL);
		return (returnstr);
	}
	...
	if (!returnstr)
		return (NULL);
	gnl_memcpy(returnstr, staticbuffer, len);
	returnstr[len] = '\0';
	return (returnstr);
}
...
void	read_into_buffer(int fd, char **staticbuffer)
{
	...
	while (read_size != 0)
	{
		...
		if (read_size <= 0)
			break ;
		buffer[read_size] = '\0';
		*staticbuffer = gnl_strjoin(*staticbuffer, buffer);
